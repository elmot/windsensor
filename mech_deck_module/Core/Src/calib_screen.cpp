...
//todo rewrite         Display::writeTextLine(strBuf, 16, 0);
    } else {
//todo rewrite         Display::writeTextLine("------------", 16, 0);
    }
    int cursorX = 0;
    int cursorY;
...
//todo rewrite     Display::setCursor(cursorX, cursorY);
}
...
void CalibScreen::prepareBackground() {
    ...
    printCalibrations();
    //todo rewrite Display::writeTextLine("Save Close Next Zoom", 0, 15);
}
...
//todo rewrite     Display::setMode(true);
}

void CalibScreen::printCalibrations() {
//todo rewrite     Display::clearText();
//todo rewrite     Display::writeTextLine("0", 14, 14);
    itoa((int) maxScreenWind(), strBuf, 10);
//todo rewrite     Display::writeTextLine(strBuf, 13, 0);
...
//todo rewrite         Display::writeTextLine(strBuf, 0, i);
    }
}

void CalibScreen::leave() {
    Screen::leave();
//todo rewrite     Display::setMode(false);
}
